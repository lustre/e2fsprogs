...
#undef EXT2_BPP
	...
	if ((ctx->lustre_devtype & LUSTRE_TYPE) == LUSTRE_MDS) {
		if (!(ctx->options & E2F_OPT_READONLY) &&
		    e2fsck_lfsck_remove_pending(ctx, block_buf))
			return;
		if (e2fsck_lfsck_check_layout(ctx, block_buf) != 0) {
			com_err(ctx->program_name, 0,
				_("this lfsck not allowed on this filesystem"));
			com_err(ctx->program_name, 0,
				_("see 'lctl lfsck_start' to check Lustre"));
			ctx->flags |= E2F_FLAG_ABORT;
			goto endit;
		}
	}
