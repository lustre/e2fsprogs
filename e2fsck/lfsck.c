...
{
	VERBOSE(2, "save duplicate object %u:"DOSTID" FID "DFID"\n",
		mds_obj->mds_ostidx, POSTID(&mds_obj->mds_oi),
		...
		for (i = 1; i < num_dup; i++) {
			...
			if (!lfsck_fidcmp(&mds_obj.mds_fid, &mds_obj2.mds_fid)){
				log_write("%s: [%u] hard link on FID "DFID" is"
					  " not a duplicate object "DOSTID"\n",
					  progname, ost_idx,
					  PFID(&mds_obj.mds_fid),
					  POSTID(&mds_obj.mds_oi));
				continue;
			}
			      ...
			      __u32 ost_idx, char *path)
{
	...
	if (!lfsck_create) {
		log_write("[%u]: %s object %s "DOSTID" not created\n", ost_idx,
	...
	if (fd < 0) {
		rc = -errno;
		log_write("[%u]: FAILED to open %s missing obj "DOSTID"\n",
			  ost_idx, path, POSTID(oi));
		fix_failed++;
		return rc;
	}
	...
	if (rc) {
		...
		log_write("[%u]: recreated %s missing obj "DOSTID"\n",
			  ost_idx, path, POSTID(oi));
		fixed++;
	}
	close(fd);
		    ...
		    DB *mds_direntdb, DB *mds_sizeinfodb)
{
	...
	while ((rc = dbcp->c_get(dbcp, &key, &data, DB_NEXT)) == 0) {
		...
		if (mds_hdr->mds_flags & E2F_OPT_READONLY &&
			...
			VERBOSE(2, "[%u] skipping MDS FID "DFID": object "DOSTID
				" > max "LPU64"\n", ost_idx,
				PFID(&mds_obj.mds_fid), POSTID(&mds_obj.mds_oi),
				max_objid);
			continue;
		}
		...
		if (rc != 0) {
			...
			if (rc == DB_NOTFOUND) {
				...
				log_write("[%u]: error looking up object "DOSTID
					  ": %s\n", ost_idx,
					  POSTID(&mds_obj.mds_oi),
					  db_strerror(rc));
				rc = -EINVAL;
		...
		if (lfsck_calc_size(&mds_obj, &ost_obj, mds_sizeinfodb)) {
			log_write("[%u]: error updating file size for object "
				  DOSTID": %s\n", ost_idx,
				  POSTID(&mds_obj.mds_oi), strerror(rc));
			rc = -EINVAL;
			goto out;
		}
	}
 ...
 */
int lfsck_fix_orphan(__u32 ost_idx, struct ostid *oi,
		     struct obd_uuid *uuid, int delete)
{
	...
	lum = malloc(lum_size);
	if (lum == NULL) {
		log_write("%s: out of memory on EA (%u) orphan %u:"DOSTID"\n",
			  progname, lum_size, ost_idx, POSTID(oi));
		return -ENOMEM;
	}

	memset(file, 0, PATH_MAX);
	sprintf(file, "%s/%s-"DOSTID, lostandfounddir, uuid->uuid, POSTID(oi));

	fd = open(file, O_CREAT|O_EXCL|O_LOV_DELAY_CREATE, 0600);
	if (fd < 0) {
		rc = -errno;
		log_write("%s: unable to open %s for orphan %u:"DOSTID": %s\n",
			  progname, file, ost_idx, POSTID(oi), strerror(-rc));
		goto out_free;
	}
	...
	memcpy(&lum->lmm_objects[0].l_ost_oi, oi, sizeof(*oi));
	...
	if (rc != 0 || delete) {
		...
		if (err != 0 && errno != ENOENT) {
			rc = rc ? rc : -errno;
			log_write("%s: failed to unlink %s for orphan %u:"DOSTID
				  ": %s\n", progname, file, ost_idx,
				  POSTID(oi), strerror(-rc));
		}
	}
out_free:
	free(lum);
		    ...
		    struct obd_uuid *uuid, __u64 last_id)
{
	...
	while ((rc = dbcp->c_get(dbcp, &key, &data, DB_NEXT)) == 0) {
		...
		if (oi->oi_id > last_id) {
			VERBOSE(2, "[%u] skipping objid "DOSTID" > "LPU64"\n",
				ost_idx, POSTID(oi), last_id);
			continue;
		}
		VERBOSE(2, "[%u] processing objid "DOSTID"\n", ost_idx,
			POSTID(oi));
		...
		if (rc == 0) {
			VERBOSE(2, "[%u] found object "DOSTID" reference\n",
				ost_idx, POSTID(oi));
			continue;
		}
		...
		if (ost_obj.ost_size == 0) {
			...
			VERBOSE(1, "[%u] zero-length orphan objid "DOSTID"\n",
				ost_idx, POSTID(oi));
		...
		if (lfsck_save || lfsck_delete) {
			...
			if (rc) {
				log_write("lfsck: [%u]: failed to fix orphan "
					  "object "DOSTID", "LPU64" bytes\n",
					  ost_idx, POSTID(oi), ost_obj.ost_bytes);
				fix_failed++;
			} else {
				log_write("lfsck: [%u]: pass3 %s orphan object "
					  DOSTID", "LPU64" bytes\n", ost_idx,
					  lfsck_save ? "saved" : "unlinked",
					  POSTID(oi), ost_obj.ost_bytes);
				fixed++;
			}
		} else {
			error++;
				    ...
				    const char *path)
{
	...
	VERBOSE(2,"[%u] check duplicate FID "DFID" object "DOSTID"\n for\t%s\n",
		dup->ld_ost_idx, PFID(&dup->ld_mds_fid), POSTID(&dup->ld_oi),
		path);
	...
	if (lum == NULL) {
		log_write("%s: out of memory allocating LOV EA (%d)\n",
			  progname, (int)LOV_EA_MAX(lum));
		return -ENOMEM;
	}
	...
	for (i = 0; i < lum->lmm_stripe_count; i++, loi++) {
		if (loi->l_ost_idx == dup->ld_ost_idx &&
		    ostid_id(&loi->l_ost_oi) == ostid_id(&dup->ld_oi) &&
		    ostid_seq(&loi->l_ost_oi) == ostid_seq(&dup->ld_oi))
			break;
	}

	if (i == lum->lmm_stripe_count) {
		log_write("%s: cannot find object %u:"DOSTID" in\n\t%s\n",
			  progname, dup->ld_ost_idx, POSTID(&dup->ld_oi), path);
		rc = -EBADF;
		goto out;
	}
			       ...
			       DB *mds_direntdb, const char *path)
{
	...
	if (!lfsck_create) {
		VERBOSE(1, "%s: [%u]: not duplicating FID "DFID
			" object "DOSTID" by request\n  on\t%s\n",
			progname, dup->ld_ost_idx, PFID(&dup->ld_mds_fid),
			POSTID(&dup->ld_oi), path);
		return 0;
	}
	...
	if (rc) {
		rc = -errno;
		log_write("%s: duplicating object for %u:"DOSTID" %s: %s\n",
			  progname, dup->ld_ost_idx, POSTID(&dup->ld_oi),
			  path, strerror(-rc));
		goto out;
	}
	...
	rc = asprintf(&dup->ld_link, "%s/%u-"DOSTID"-"DFID":%s", dupedir,
		      dup->ld_ost_idx, POSTID(&dup->ld_oi),
	...
	if (rc) {
		...
		log_write("%s: [%u]: fixed duplicate FID "DFID" object "
			  ...
			  PFID(&dup->ld_mds_fid), POSTID(&dup->ld_oi), path);
	}
out:
	if (rc) {
		VERBOSE(2, "unlink %s\n", path_tmp);
...
int lfsck_run_checks()
{
	...
	if (mds_hdr == NULL) {
		log_write("%s: out of memory allocating DB header (%d)\n",
			  progname, (int)sizeof(*mds_hdr));
		rc = -ENOMEM;
		goto out;
	}
	memset(&key, 0, sizeof(key));
