			 ...
			 struct lov_user_md *lmm, struct lustre_mdt_attrs *lma)
{
	...
	for (i = 0; i < lmm->lmm_stripe_count; i++, loi++) {
		...
		mds_ent.mds_oi = *(struct ostid *)&loi->l_ost_oi;
		mds_ent.mds_ostidx = ost_idx;
		mds_ent.mds_ostoffset = i;

		if (ostid_id(&mds_ent.mds_oi) > ofile_ctx->max_id) {
			if (ofile_ctx->have_max_id) {
				DEBUG(ctx,
				      "[%d] skip obj "LPU64" > max "LPU64"\n",
				      ost_idx, ostid_id(&mds_ent.mds_oi),
				      ofile_ctx->max_id);
				continue;
			}
			ofile_ctx->max_id = ostid_id(&mds_ent.mds_oi);
		}
		...
		dbp = ofile_ctx->dbp;
#if 0
		DEBUG(ctx, "OST[%u]: inode FID "DFID" oi "DOSTID"\n", ost_idx,
		      PFID(&mds_ent.mds_fid), POSTID(&mds_ent.mds_oi));
#endif
		rc = dbp->put(dbp, NULL, &key, &data, 0);
		if (rc != 0) {
			 ...
			 struct lustre_mdt_attrs **lma)
{
	...
	if (entry->e_name_len == strlen(LUSTRE_XATTR_MDT_LOV) &&
	    strncmp(entry->e_name, LUSTRE_XATTR_MDT_LOV,
		    entry->e_name_len) == 0) {
		if (*lmm) {
			ext2fs_free_mem(&ea);
			return -EINVAL;
	...
	} else if (entry->e_name_len == strlen(LUSTRE_XATTR_MDT_LMA) &&
		   strncmp(entry->e_name, LUSTRE_XATTR_MDT_LMA,
			   entry->e_name_len) == 0) {
		if (*lma) {
			ext2fs_free_mem(&ea);
			return -EINVAL;
			   ...
			   int blocksize, char *buf, void *priv_data)
{
	...
	objent.ost_oi.oi_id = strtoul(name, NULL, 0);
	if (objent.ost_oi.oi_id == ULONG_MAX) {
		lctx->status = 1;
			  ...
			  int blocksize, char *buf, void *priv_data)
{
	...
	DBT key, data;
	DB *dbp = lctx->outdb;
	int file_type = (dirent->name_len >> 8) & EXT2_FT_MASK;
	int rc = 0;

	DEBUG(lctx->ctx, "MDT: inode %u, file %.*s, type %u\n", dirent->inode,
	      dirent->name_len & EXT2_NAME_LEN, dirent->name, file_type);
	...
	DEBUG(lctx->ctx, "MDT: dirfid "DFID" child "DFID" file %.*s\n",
	      ...
	      dirent->name_len & EXT2_NAME_LEN, dirent->name);
